...
use super::setup::{SHADOW_Z, ACTOR_Z};
...
pub fn spawn_enemy_kind(
    ...
    let (mut hp, mut damage, speed, color, mut size) = enemy_stats(kind, floor);
    if elite {
        hp *= 2;
    ...
    let (sprite, anim, tint) = if let Some(m) = art.monster_for(kind) {
        ...
        a.moving = true;
        (m.sprite(m.walk.0, scale, Color::WHITE), a, Color::WHITE)
    } else {
        let mut a = CatAnimation::new(CAT_IDLE, CAT_WALK, None, 8);
        a.moving = true;
        (art.cat_sprite(CAT_WALK.0, size, color), a, color)
    };
    ...
    if elite {
        entity.with_child((
            art.image_sprite(&art.orb, Vec2::splat(size * 1.5), color.with_alpha(0.32)),
...
fn enemy_stats(kind: EnemyKind, floor: u32) -> (i32, i32, f32, Color, f32) {
    ...
    let ff = floor as f32;
    match kind {
        EnemyKind::Hunter => (2 + f, 1 + f / 4, 130.0 + ff * 6.0, Color::srgb(1.0, 0.66, 0.45), 74.0),
        EnemyKind::Bruiser => (5 + f * 2, 2 + f / 3, 92.0 + ff * 4.0, Color::srgb(0.86, 0.40, 0.40), 88.0),
        EnemyKind::Seeker => (3 + f, 1 + f / 3, 150.0 + ff * 7.0, Color::srgb(0.72, 0.55, 1.0), 74.0),
        EnemyKind::Boss => (90 + f * 30, 5 + f / 2, 100.0 + ff * 2.0, Color::srgb(1.0, 0.34, 0.40), 156.0),
        EnemyKind::Charger => (6 + f * 2, 3 + f / 2, 120.0 + ff * 4.0, Color::srgb(0.92, 0.66, 0.34), 100.0),
        EnemyKind::Bomber => (2 + f / 2, 4 + f / 2, 150.0 + ff * 8.0, Color::srgb(0.95, 0.40, 0.28), 64.0),
        EnemyKind::Kitten => (1, 1, 170.0 + ff * 8.0, Color::srgb(0.82, 0.82, 0.88), 46.0),
        EnemyKind::Splitter => (4 + f, 2 + f / 3, 96.0 + ff * 4.0, Color::srgb(0.52, 0.86, 0.52), 80.0),
        EnemyKind::Caster => (3 + f, 1 + f / 3, 120.0 + ff * 5.0, Color::srgb(0.50, 0.62, 1.0), 72.0),
        EnemyKind::Summoner => (5 + f, 1 + f / 4, 105.0 + ff * 4.0, Color::srgb(0.92, 0.50, 0.92), 82.0),
        EnemyKind::Scratcher => (2 + f / 2, 1 + f / 4, 180.0 + ff * 10.0, Color::srgb(0.8, 0.2, 0.25), 56.0),
        EnemyKind::Chonker => (15 + f * 3, 2 + f / 3, 70.0 + ff * 2.0, Color::srgb(0.9, 0.7, 0.4), 110.0),
        EnemyKind::ShadowCat => (4 + f, 2 + f / 3, 140.0 + ff * 6.0, Color::srgb(0.4, 0.2, 0.6), 68.0),
    }
}