...
pub fn perform_attack(
    ...
    match enemy.kind {
        ...
        EnemyKind::Charger => {}
        EnemyKind::Bomber => {
            explode(commands, art, pos, enemy.damage + 2, 120.0, player_pos, player_hp, shake, invuln);
            play(commands, &sfx.explosion, 0.6);
            commands.entity(entity).despawn();
        }
        EnemyKind::Seeker => {
            spawn_projectile(commands, art, pos + dir * 24.0, dir * 250.0, ProjectileOwner::Enemy, enemy.damage);
        }
        EnemyKind::Boss => {
            ...
            if hp_percent > 0.66 {
                for a in [-0.25_f32, 0.0, 0.25] {
                    let d = rotate(dir, a);
                    spawn_projectile(commands, art, pos + d * 28.0, d * 300.0, ProjectileOwner::Enemy, enemy.damage + 1);
                }
            } else if hp_percent > 0.33 {
                for a in [-0.4_f32, -0.2, 0.0, 0.2, 0.4] {
                    let d = rotate(dir, a);
                    spawn_projectile(commands, art, pos + d * 28.0, d * 340.0, ProjectileOwner::Enemy, enemy.damage + 1);
                }
                enemy.action_cd = Timer::from_seconds(0.5, TimerMode::Once);
            } else {
                for a in [-0.5_f32, -0.33, -0.16, 0.0, 0.16, 0.33, 0.5] {
                    let d = rotate(dir, a);
                    spawn_projectile(commands, art, pos + d * 28.0, d * 380.0, ProjectileOwner::Enemy, enemy.damage + 2);
                }
                enemy.action_cd = Timer::from_seconds(0.35, TimerMode::Once);
            }
        }
        EnemyKind::Caster => {
            ...
            for i in 0..n {
                ...
                let d = rotate(dir, a);
                spawn_projectile(commands, art, pos + d * 22.0, d * 230.0, ProjectileOwner::Enemy, enemy.damage);
            }
        }
        EnemyKind::Summoner => {
            if enemy.ammo > 0 {
        ...
        EnemyKind::Scratcher => {
            if distance < 50.0 { *player_hp -= enemy.damage; }
        }
        EnemyKind::Chonker => {
